...
export default function Envios() {
  ...
  // YA NO USAMOS ESTADO DE PRODUCTOS
    ...
    <div className="space-y-4" style={{ padding: "1.5rem" }}>
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={openCreate}
          style={{
            background: "#4F46E5", color: "white", padding: "8px 14px", borderRadius: "6px", border: "none", cursor: "pointer"
          }}
        >
          + Nuevo
        </button>
      ...
      <div style={{ marginTop: 15, display: "flex", gap: 10 }}>
        ...
        <button
          onClick={load}
          style={{ padding: "8px 14px", borderRadius: 6, background: "#f3f4f6", border: "1px solid #ddd", cursor: "pointer" }}
        >
          Recargar
        </button>
      </div>

      <div style={{ marginTop: 20, background: "white", borderRadius: "12px", boxShadow: "0px 2px 6px rgba(0,0,0,0.1)", padding: 10, overflowX: "auto" }}>
              ...
              filtered.map((row) => (
                <tr key={row.id_envio} style={{ borderTop: "1px solid #eee" }}>
                  ...
                  <td style={{ padding: "10px" }}>
                     ...
                     <span style={{fontSize:"0.85em", color:"#666"}}>(${row.Pedido?.total || 0})</span>
                  </td>
                  <td style={{ padding: "10px" }}>{row.responsable?.nombre_empleado || "Sin asignar"}</td>
                  <td style={{ padding: "10px" }}>
                    <span style={{
                      padding: "4px 8px", 
                      borderRadius: "4px",
      ...
      {modalOpen && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.3)", display: "grid", placeItems: "center", zIndex: 50 }}>
          <form onSubmit={save} style={{ background: "white", padding: "1.5rem", borderRadius: 12, width: "100%", maxWidth: 500, boxShadow: "0 5px 20px rgba(0,0,0,0.2)" }}>
            <h3 style={{ marginBottom: 15, fontSize: "1.25rem" }}>{editingId ? "Editar envío" : "Nuevo envío"}</h3>
            ...
            >
              ...
              {pedidos.map(p => (
                <option key={p.id_pedido} value={p.id_pedido}>#{p.id_pedido} - {p.fecha_pedido} ({p.status})</option>
              ))}
            </select>
            ...
            >
              ...
              {empleados.map(e => (
                <option key={e.id_empleado} value={e.id_empleado}>{e.nombre_empleado} ({e.rol})</option>
              ))}
            </select>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 10 }}>
               <button type="button" onClick={() => setModalOpen(false)} style={{ background: "#e5e7eb", padding: "8px 16px", borderRadius: 6, border: "none", cursor: "pointer" }}>Cancelar</button>
               <button type="submit" style={{ background: "#4F46E5", color: "white", padding: "8px 16px", borderRadius: 6, border: "none", cursor: "pointer" }}>Guardar</button>
            </div>
          </form>
        </div>
      )}
