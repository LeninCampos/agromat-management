...
export default function Suministros() {
  ...
  const handleFileChange = async (e) => {
    ...
    // Interfaz actualizada con selección de moneda
    const { value: formValues } = await Swal.fire({
      title: 'Importar Excel',
      html: `
        <div style="text-align: left; font-size: 0.9rem; color: #555;">
          <label style="display:block; margin-bottom: 4px; font-weight:600;">Proveedor</label>
          ...
          <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px;">
            ...
            <div>
                ...
                <input id="swal-hora" type="time" class="swal2-input" style="margin:0; width: 100%;" value="${new Date().toTimeString().slice(0, 5)}">
            </div>
          </div>
        </div>
      `,
      ...
      preConfirm: () => {
        ...
        const moneda = document.querySelector('input[name="swal-moneda"]:checked').value;
    ...
    if (formValues) {
      ...
      // Enviamos moneda y tasa
      formData.append("moneda", formValues.moneda);
      formData.append("tasa_cambio", formValues.tasa_cambio);
    ...
    <div className="space-y-4" style={{ padding: "1.5rem" }}>
      <div className="flex items-center justify-between">
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          ...
          <button onClick={handleImportClick} style={{ background: '#10B981', color: 'white', padding: "8px 14px", borderRadius: "6px", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: "6px", fontWeight: 500 }}>
            📄 Importar Excel
          </button>
            ...
            {loading ? (
              ...
              filteredItems.map(row => (
                <tr key={row.id_suministro} style={{ borderTop: "1px solid #eee" }}>
                  ...
                  <td style={{ padding: "12px" }}>{row.fecha_llegada}<br /><span style={{ fontSize: "0.8em", color: "#666" }}>{row.hora_llegada}</span></td>
                  <td style={{ padding: "12px" }}>{row.Proveedor?.nombre_proveedor}</td>
                  <td style={{ padding: "12px" }}>{row.transportista || "-"}</td>
                  <td style={{ padding: "12px" }}>{row.Empleado?.nombre_empleado || <span style={{ color: "#999" }}>Sin asignar</span>}</td>
                  <td style={{ padding: "12px", textAlign: "center" }}><span style={{ background: "#e0f2fe", color: "#0369a1", padding: "3px 8px", borderRadius: "10px", fontWeight: "bold" }}>{row.Suministras?.length || 0}</span></td>
                  <td style={{ padding: "12px", textAlign: "center" }}>
                    <div style={{ display: "flex", gap: "8px", justifyContent: "center" }}>
                      <button onClick={() => openDetails(row)} style={{ background: "#3B82F6", color: "white", padding: "5px 10px", borderRadius: "6px", border: "none", cursor: "pointer", fontSize: "0.8rem" }}>Detalles</button>
                      <button onClick={() => remove(row.id_suministro)} style={{ background: "#DC2626", color: "white", padding: "5px 10px", borderRadius: "6px", border: "none", cursor: "pointer", fontSize: "0.8rem" }}>Eliminar</button>
                    </div>
                  </td>
                </tr>
              ))
      ...
      {modalOpen && (
        <div className="agromat-modal-backdrop">
          <div className="agromat-modal-card" style={{ maxWidth: "600px" }}>
            <div className="agromat-modal-header"><h2>Registrar Ingreso Manual</h2><button onClick={() => setModalOpen(false)} className="agromat-modal-close">✕</button></div>
            <form onSubmit={save} className="agromat-modal-body">
              <div className="agromat-form-grid">
                <div className="agromat-form-field"><label>Fecha</label><input type="date" className="agromat-input" required value={form.fecha_llegada} onChange={e => setForm({ ...form, fecha_llegada: e.target.value })} /></div>
                <div className="agromat-form-field"><label>Hora</label><input type="time" className="agromat-input" required value={form.hora_llegada} onChange={e => setForm({ ...form, hora_llegada: e.target.value })} /></div>
                <div className="agromat-form-field agromat-full-row"><label>Proveedor</label><select className="agromat-select" required value={form.id_proveedor} onChange={e => setForm({ ...form, id_proveedor: e.target.value })}><option value="">-- Selecciona --</option>{proveedores.map(p => <option key={p.id_proveedor} value={p.id_proveedor}>{p.nombre_proveedor}</option>)}</select></div>
                <div className="agromat-form-field"><label>Transportista</label><input type="text" className="agromat-input" placeholder="Ej. DHL..." value={form.transportista} onChange={e => setForm({ ...form, transportista: e.target.value })} /></div>
                <div className="agromat-form-field"><label>Recibió</label><input type="text" className="agromat-input" value={user ? user.nombre : "Cargando..."} disabled style={{ backgroundColor: "#f3f4f6", cursor: "not-allowed", color: "#555" }} /></div>
              </div>
              <hr style={{ margin: "20px 0", border: "0", borderTop: "1px solid #eee" }} />
              <div style={{ display: "flex", gap: "10px", alignItems: "flex-end" }}>
                <div style={{ flex: 1 }}><label style={{ fontSize: "0.8rem", fontWeight: 600 }}>Producto</label><select className="agromat-select" style={{ width: "100%" }} value={newItem.id_producto} onChange={e => setNewItem({ ...newItem, id_producto: e.target.value })}><option value="">Buscar...</option>{productos.map(p => <option key={p.id_producto} value={p.id_producto}>{p.nombre_producto} (Stock: {p.stock})</option>)}</select></div>
                <div style={{ width: "80px" }}><label style={{ fontSize: "0.8rem", fontWeight: 600 }}>Cant.</label><input type="number" className="agromat-input" min="1" style={{ width: "100%" }} value={newItem.cantidad} onChange={e => setNewItem({ ...newItem, cantidad: parseInt(e.target.value) || 0 })} /></div>
              ...
              <div style={{ marginTop: "15px", background: "#f9fafb", padding: "10px", borderRadius: "8px", maxHeight: "150px", overflowY: "auto" }}>
                {form.items.length === 0 ? <p style={{ textAlign: "center", color: "#888", fontSize: "0.9rem" }}>Sin productos</p> : (
                  <table style={{ width: "100%", fontSize: "0.9rem" }}>
                    <tbody>{form.items.map(item => (<tr key={item.id_producto} style={{ borderBottom: "1px solid #eee" }}><td style={{ padding: "4px" }}>{item.nombre}</td><td style={{ padding: "4px", fontWeight: "bold" }}>+{item.cantidad}</td><td style={{ textAlign: "right" }}><button type="button" onClick={() => eliminarProducto(item.id_producto)} style={{ color: "red", border: "none", background: "none", cursor: "pointer" }}>✕</button></td></tr>))}</tbody>
                  </table>
                )}
              </div>
              <div className="agromat-modal-footer"><button type="button" className="agromat-btn-secondary" onClick={() => setModalOpen(false)}>Cancelar</button><button type="submit" className="agromat-btn-primary">Registrar Ingreso</button></div>
            </form>
          </div>
        </div>
      )}
